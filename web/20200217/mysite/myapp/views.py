...
def listuser(request):
    if request.method == 'GET':
        ...
        q = request.GET.get('q','') #q가 없다면 ''로 표시함
        fun = request.GET.get('q','') #q가 없다면 ''로 표시함
        if q == '':
        ...
        #u = User(userid,name,age,hobby)
        u = User.objects.create(userid=userid, name=name, age=age, hobby=hobby)
        u.save()
        return redirect('/listuser')
