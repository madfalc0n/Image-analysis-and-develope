...
def listsql(request, category, page):
    ...
    return render(request, 'myboard/list2.html', {'data': subs})
...
def listimg(request):
    ...
    cursor = connection.cursor()
    cursor.execute(sql)
    datas = [dict(zip([col[0] for col in cursor.description], row)) for row in cursor.fetchall()] 
    context = {"data":datas, "username":username}    
    print(context)

    return render(request,  "myboard/imglist.html", context)
