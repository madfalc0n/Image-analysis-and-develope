...
pretrained_path = {'se_resnext50_32x4d': '/home/lab05/kaggle_dir/project_socket_server/se_resnext50_32x4d-a260b3a4.pth'}
...
def inference(model, test_loader, device):
    ...
    test_loader = torch.unsqueeze(test_loader, 0)
    test_loader = test_loader.to(device)
    with torch.no_grad():
        y_preds = model(test_loader)
...
def getImage(file_path, transform=None):
...
def submit(coefficients, file_path, device):
    print('run inference')
    test_loader = getImage(file_path, transform=get_transforms(data='valid'))
    probs = []
    print(test_loader.shape)
    for fold in range(CFG.n_fold):
        ...
        weights_path = f'/home/lab05/kaggle_dir/project_socket_server/fold{fold}_se_resnext50.pth'
        print("check2")
...
def var(file_path):
    ...
    preds = np.array(pd.read_csv('/home/lab05/kaggle_dir/project_socket_server/p_data.csv')).flatten().astype(np.float64)
    valid_labels = np.array((pd.read_csv('/home/lab05/kaggle_dir/project_socket_server/v_data.csv'))).flatten().astype(np.float32)
    ...
    submission = submit(coefficients, file_path, device)
